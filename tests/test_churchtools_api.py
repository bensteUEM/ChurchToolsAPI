...
class TestsChurchToolsApi(unittest.TestCase):
    ...
    def test_create_edit_delete_song(self) -> None:
        ...
        messages = [
            f"INFO:churchtools_api.songs:Did not find song ({song_id}) with CODE 404",
        ]
        assert messages == cm.output
    ...
    def test_add_remove_song_tag(self) -> None:
        ...
        assert self.api.contains_song_tag(408, 53)
        with self.assertNoLogs(level="INFO"):
            response = self.api.remove_song_tag(408, 53)
        assert response.status_code == 200

        self.api.ajax_song_last_update = None
        assert not self.api.contains_song_tag(408, 53)
